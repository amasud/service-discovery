...
function() {
  ...
  c.runSupplyAnalysis = function() {
    ...
    c.server.get({ action: 'loadSupplyResults' }).then(function(r) {
      c.data.supplyData = r.data.supplyData; c.running = false; c.goReviewRegistry();
    });
  };
  ...
  c.runDemandAnalysis = function() {
    ...
    c.server.get({ action: 'loadGaps' }).then(function(r) {
      c.data.gapData = r.data.gapData; c.running = false; c.goReviewUnmatched();
    });
  };
    ...
    for (var ci = 0; ci < c.data.supplyData.companies.length; ci++) {
      for (var pi = 0; pi < c.data.supplyData.companies[ci].products.length; pi++) {
        ...
        for (var ti = 0; ti < c.data.supplyData.companies[ci].products[pi].topics.length; ti++) {
          topics++; kb += c.data.supplyData.companies[ci].products[pi].topics[ti].kb;
